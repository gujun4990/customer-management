use customer_management_lib::crm::{
    create_lead_for_session, create_lead_followup_for_session,
    list_lead_followups_for_session, CreateLeadInput,
};
...
fn test_create_lead_followup() {
    ...
    tauri::async_runtime::block_on(async move {
        ...
        let lead = create_lead_for_session(
            ...
            &session,
            CreateLeadInput {
                name: "FollowUp Test Company".into(),
                phone: "555-0004".into(),
        ...
        let follow_up = create_lead_followup_for_session(
            &pool,
            &session,
            &lead_id,
        ...
        assert!(matches!(&follow_up.target, customer_management_lib::domain::FollowUpTarget::Lead(id) if *id == lead_id));
...
#[test]
fn test_list_lead_followups_sorted_newest_first() {
    let test_database_url = match std::env::var("TEST_DATABASE_URL") {
        Ok(value) => value,
        Err(_) => return,
    };
    ...
    tauri::async_runtime::block_on(async move {
        ...
        let follow_ups =
            list_lead_followups_for_session(&pool, &session, &lead_id)
                .await
...
#[test]
fn test_non_owner_cannot_create_followup() {
    let test_database_url = match std::env::var("TEST_DATABASE_URL") {
        Ok(value) => value,
        Err(_) => return,
    };
    ...
    tauri::async_runtime::block_on(async move {
        ...
        let result = create_lead_followup_for_session(
            &pool,
            &bob_session,
            &lead_id,
